...
curl -k "https://www.ebi.ac.uk/gwas/api/search/downloads/studies_alternative" > all_studies.tsv
...
printf "pubmedId\ttitle\tauthor_s\tauthorAscii_s\tpublication\tpublicationDate\taccessionId\tassociationCount\ttraitName_s\tmappedLabel\tmappedUri\tsample_size\n" > "$STUDY"_metadata_ebi.tsv
while IFS="" read -r gwas_id || [ -n "$gwas_id" ]; do
    grep -e "$gwas_id" all_studies_ss.tsv > selected_studies_metadata_original.tsv
    awk 'BEGIN{OFS="\t";FS="\t"}{print $2,$7,$3,$3,$5,$4,$15,$12,$8,$13,$14,$17}' selected_studies_metadata_original.tsv >> "$STUDY"_metadata_ebi.tsv
done < downloaded_studies.txt

# Print sample_size as a separated file to be able to add it to the VCF GWAS column later
awk -F "\t" '{if (NR==2) print $12}' "$STUDY"_metadata_ebi.tsv > "$STUDY"_sample_size.txt
