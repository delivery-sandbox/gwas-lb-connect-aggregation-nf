...
def modify_header(header, stnd, beta2or, or2beta):
    ...
    for e in header:
        if e[:8] == '##FORMAT':
            ...
            if or2beta:
                new_e = '##FORMAT=<ID=BETA_,Number=1,Type=Float,Description="BETA converted from OR">\n'
                e = new_e + e
                format_written = True
            if beta2or:
                new_e = '##FORMAT=<ID=OR_,Number=1,Type=Float,Description="OR converted from BETA">\n'
                e = new_e + e
                format_written = True
            if stnd:
                new_e = '##FORMAT=<ID=STD_BETA,Number=1,Type=Float,Description="Standardised BETA">\n'
                new_e2 = '##FORMAT=<ID=STD_BETA_SE,Number=1,Type=Float,Description="Standardised BETA SE">\n'
                e = new_e + new_e2 + e
                format_written = True
        new_header.append(e)
    return new_header
...
def modify_record(line, stnd, beta2or, or2beta, median_phe_se, beta_col,
    ...
    if stnd:
        ...
        new_line[8] += ':STD_BETA:STD_BETA_SE'
